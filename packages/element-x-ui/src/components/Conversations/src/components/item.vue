    ...
    >
        <div class="el-x-conversation-content">
            ...
            <div class="el-x-conversation-content-main">
                <slot name="label">
                    <!-- 前缀图标 -->
                    <span v-if="prefixIconRender" class="el-x-conversation-prefix-icon">
                        <i :class="prefixIcon"></i>
                    </span>

                    <!-- 标签和时间戳 -->
                    <div class="el-x-conversation-label-container">
                        <el-tooltip v-if="showTooltip && isTextOverflow(item.label)" :content="item.label" :placement="tooltipPlacement" :offset="tooltipOffset" effect="dark">
                            <span class="el-x-conversation-label" :class="{ 'text-gradient': isTextOverflow(item.label) }" :style="labelStyle">{{ item.label }}</span>
                        </el-tooltip>
                        <span v-else class="el-x-conversation-label" :class="{ 'text-gradient': isTextOverflow(item.label) }" :style="labelStyle">{{ item.label }}</span>
                    </div>
                </slot>
            </div>

            <!-- 后缀图标 -->
            <span v-if="suffixIcon" class="el-x-conversation-suffix-icon">
                <i :class="suffixIcon"></i>
            </span>

            <!-- 菜单区域 - 只在hover或active状态下显示 -->
            <div v-if="shouldShowMenu && showBuiltInMenu">
                <div v-if="menu && menu.length" ref="menuButtonRef" class="el-x-conversation-dropdown-more" @click.stop>
                    <el-dropdown
                        trigger="click"
                        :placement="menuPlacement"
                        :offset="menuOffset"
                        :teleported="menuTeleported"
                        :popper-class="menuClassName ? `el-x-conversation-dropdown-menu ${menuClassName}` : 'el-x-conversation-dropdown-menu'"
                    ...
                    >
                        <span class="el-dropdown-link">
                            <slot name="more-filled" :item="item" :is-hovered="item.disabled ? false : isHovered" :is-active="active" :is-menu-opened="isShowMenuBtn" :is-disabled="item.disabled">
                                <i class="el-icon-more el-x-conversation-dropdown-more-icon"></i>
                            </slot>
                        </span>
                        <el-dropdown-menu slot="dropdown" :style="mergedMenuStyle">
                            <slot name="menu">
                                <el-dropdown-item
