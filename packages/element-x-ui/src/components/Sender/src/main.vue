    ...
    >
        ...
        >
            ...
            <transition name="slide">
                <div v-if="visiableHeader" class="el-x-sender-header-wrap">
                    <div v-if="$slots.header" class="el-x-sender-header">
                        <slot name="header"></slot>
                    </div>
                </div>
            </transition>
            <!-- 内容容器 内置变体逻辑 -->
            <div class="el-x-sender-content" :class="{ 'content-variant-updown': variant === 'updown' }" @mousedown="onContentMouseDown">
                <!-- Prefix 前缀 -->
                <div v-if="$slots.prefix && variant === 'default'" class="el-x-sender-prefix">
                ...
                />
                <!-- 操作列表 -->
                <div v-if="variant === 'default'" class="el-x-sender-action-list">
                    <slot name="action-list">
                        <div class="el-x-sender-action-list-presets">
                            <send-button v-if="!loading" :disabled="isSubmitDisabled" @submit="submit"></send-button>

                            <loading-button v-if="loading" @cancel="cancel"></loading-button>

                            <speech-button v-if="!speechLoading && allowSpeech" @click="startRecognition"></speech-button>

                            <speech-loading-button v-if="speechLoading && allowSpeech" @click="stopRecognition"></speech-loading-button>

                            <clear-button v-if="clearable" @clear="clear"></clear-button>
                        </div>
                    </slot>
                </div>

                <!-- 变体样式 -->
                <div v-if="variant === 'updown' && showUpdown" class="el-x-sender-updown-wrap">
                    <!-- 变体 updown： Prefix 前缀 -->
                    <div v-if="$slots.prefix" class="el-x-sender-prefix">
                        <slot name="prefix"></slot>
                    </div>
            ...
            <!-- 底部容器 -->
            <transition name="slide">
                <div v-if="$slots.footer" class="el-x-sender-footer">
