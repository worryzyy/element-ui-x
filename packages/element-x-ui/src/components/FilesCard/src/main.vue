<template>
  ...
  >
    <div
      v-if="status === 'uploading' && !(isSquareVariant && isImageFile)" class="el-x-filescard-progress"
      :style="{ width: `${percent || 0}%` }"
    />

    <slot v-if="$slots.icon || _fileType" name="icon" :item="propsData">
      <component
        :is="svgIconMap[_fileType]" v-if="!isImageFile" class="el-x-filescard-icon"
      ...
      <template v-if="isImageFile">
        ...
        >
          <el-image
            ...
            :src="_previewImgUrl"
            :preview-src-list="imgPreview ? [_previewImgUrl] : []" fit="cover" :show-progress="false"
            hide-on-click-modal @show="handlePreviewAction('self')"
          />

          <!-- 无预览地址时显示默认图标 -->
          <component
            :is="svgIconMap[_fileType]" v-else class="el-x-filescard-img"
            :color="iconColor || colorMap[_fileType]"
          />
          ...
          <transition name="fade">
            <slot
              ...
              name="image-preview-actions" :item="{ ...propsData, prefix: namePrefix, suffix: nameSuffix }"
            >
    ...
    <slot
      v-if="($slots.content || name || description) && !(isSquareVariant && isImageFile)" name="content"
      :item="propsData"
    >
      <div class="el-x-filescard-content">
        <div v-if="name" class="el-x-filescard-name">
          <slot name="name-prefix" :item="{ ...propsData, prefix: namePrefix, suffix: nameSuffix }">
            <div class="el-x-filescard-name-prefix">
              {{ namePrefix }}
            </div>
          </slot>
          <slot name="name-suffix" :item="{ ...propsData, prefix: namePrefix, suffix: nameSuffix }">
            <div class="el-x-filescard-name-suffix">
              {{ nameSuffix }}
            </div>
          </slot>
        </div>
        ...
        <slot name="description" :item="{ ...propsData, prefix: namePrefix, suffix: nameSuffix }">
          <div
            class="el-x-filescard-description" :class="{
...
export default {
  ...
  computed: {
    ...
    _iconSize() {
      if ((this.isSquareVariant && this.isImageFile && !this.iconSize) || (this.isSquareVariant && this.isImageFile && this.iconSize === '42px'))
        return '64px'
