<template>
    <div class="el-x-thoughtchain">
      ...
      maxWidth: maxWidth,
            ...
            <transition-group name="thought-chain" tag="div">
                <el-timeline-item
                ...
                >
                    <div v-if="!item.isCanExpand">
                        <el-x-typewriter :content="getThinkTitle(item)" :is-markdown="item.isMarkdown" :typing="item.typing" />
                    </div>
                    <el-collapse v-else-if="!item.isDefaultExpand" @change="() => handleExpand(item)">
                        <el-collapse-item :title="getThinkTitle(item)">
                            <el-x-typewriter :content="getThinkContent(item)" :is-markdown="item.isMarkdown" :typing="item.typing" />
                        </el-collapse-item>
                    </el-collapse>
                    <el-collapse v-else v-model="defaultActiveNodes" @change="() => handleExpand(item)">
                        <el-collapse-item :title="getThinkTitle(item)" :name="String(getId(item))">
                            <el-x-typewriter :content="getThinkContent(item)" :is-markdown="item.isMarkdown" :typing="item.typing" />
                        </el-collapse-item>
                    </el-collapse>

                    <template v-if="!dotIsIcon" slot="dot">
                        <div class="el-x-thoughtchain-item-dot" :style="{ margin: dotMargin }">
                            <slot name="icon" :item="item">
                                <el-button circle :type="getType(item)" :loading="isLoading(item)" :size="dotSize">
                                    ...
                                    <i v-if="!isLoading(item) && !isError(item)" class="el-icon-check"></i>
                                    <i v-if="!isLoading(item) && isError(item)" class="el-icon-close"></i>
                                </el-button>
                            </slot>
                        </div>
                    </template>
