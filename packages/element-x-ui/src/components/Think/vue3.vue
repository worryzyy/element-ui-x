...
<template>
    ...
    >
        <!-- 触发按钮 -->
        <button class="trigger" :class="[status, { disabled: !props.disabled }]" :disabled="props.disabled" @click="changeExpand">
            <span class="status-icon">
                <slot name="status-icon" :status="props.status">
                    <el-icon v-if="status === 'thinking'" class="is-loading el-icon-center">
                        <Loading />
                    </el-icon>
...
.slide-leave-active {
    ...
    transition: height var(--el-thinking-animation-duration) ease-in-out, opacity var(--el-thinking-animation-duration) ease-in-out;
    overflow: hidden;
}
...
.content pre {
    ...
    font-family: DeepSeek-CJK-patch, Inter, system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, Roboto, Noto Sans, Ubuntu, Cantarell,
        Helvetica Neue, Oxygen, Open Sans, sans-serif;
    margin: 0;
