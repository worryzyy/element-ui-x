    ...
    methods: {
      ...
      checkSelect(tag) {
        ...
        if (!this.currentSelectElm) {
          this.editorRef?.setSelectTag(this.viewKey, tag.id);
        } else {
          // 有值走标签更新方法
          this.editorRef?.updateSelectTag(this.currentSelectElm, tag);
        }
      ...
      checkUser(user) {
        this.editorRef?.customSetUser(user);
      ...
      checkTag(tag) {
        this.editorRef?.customSetTag(this.viewPrefix, tag);
