...
<template>
  <div>
    ...
    <div class="demo-controls">
      <el-button-group>
        <el-button size="small" type="primary" @click="startTyping">开始</el-button>
        <el-button size="small" type="warning" :disabled="!isTyping" @click="pauseTyping">暂停</el-button>
        <el-button size="small" type="success" :disabled="isTyping || progress >= 100" @click="continueTyping">继续</el-button>
      </el-button-group>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      typingContent: '这是一个带有打字机效果的气泡示例，可以模拟实时输入的效果。你可以通过下方的按钮控制打字过程。',
      typingConfig: {
        interval: 50,
        step: 1,
        suffix: '|'
...
<template>
  <div>
    ...
    <div class="demo-controls">
      <el-button size="small" type="primary" @click="startMarkdown">预览</el-button>
    </div>
  </div>
</template>
...
<template>
  <div>
    <el-x-bubble placement="start">
      <template #avatar>
        <el-avatar :size="40" icon="el-icon-user-solid" style="background-color: #409EFF;"></el-avatar>
      </template>
...
<template>
  <div class="chat-container">
    ...
    <div class="demo-controls">
      <el-button size="small" type="primary" @click="regenerateResponse">重新生成回答</el-button>
    </div>
  </div>
</template>
