    ...
    <div>
        <el-card class="demo-card">
            ...
            <div class="demo-block">
                <h3>基础用法</h3>
                <el-x-sender v-model="basicContent" placeholder="请输入内容..." clearable @submit="handleBasicSubmit"></el-x-sender>
            ...
            <div class="demo-block">
                <h3>不同变体</h3>
                <h4>默认变体</h4>
                <el-x-sender v-model="variantContent.default" placeholder="默认变体..." variant="default" clearable></el-x-sender>
                <h4 style="margin-top: 20px;">上下变体</h4>
                <el-x-sender v-model="variantContent.updown" placeholder="上下变体..." variant="updown" clearable></el-x-sender>
            </div>
            ...
            <div class="demo-block">
                ...
                <div class="control-row">
                    ...
                    <el-checkbox v-model="readOnlyState" style="margin-left: 20px;">只读</el-checkbox>
                </div>
                <el-x-sender v-model="stateContent" :disabled="disabledState" :read-only="readOnlyState" placeholder="禁用/只读状态..." clearable></el-x-sender>
            </div>

            <div class="demo-block">
                <h3>加载状态</h3>
                <el-x-sender v-model="loadingContent" :loading="isLoading" placeholder="模拟提交中..." clearable @submit="handleLoadingSubmit" @cancel="handleLoadingCancel"></el-x-sender>
                <div class="demo-controls">
            ...
            <div class="demo-block">
                <h3>语音输入</h3>
                <el-x-sender v-model="speechContent" placeholder="点击麦克风图标开始语音输入..." clearable allow-speech></el-x-sender>
                <div class="demo-controls">
            ...
            <div class="demo-block">
                ...
                <div class="control-row">
                    <h4>行数限制：</h4>
                    <el-slider style="width: 200px;" v-model="autoSizeConfig.maxRows" :min="1" :max="10" :step="1" :marks="{1: '1行', 5: '5行', 10: '10行'}" />
                </div>
                <el-x-sender v-model="sizeContent" :auto-size="autoSizeConfig" placeholder="尝试输入多行内容..." clearable></el-x-sender>
            </div>
                    ...
                    <template #trigger-popover="{ triggerString }">
                        <div class="trigger-popover-content">
                            <template v-if="triggerString === '@'">
                                <div class="trigger-item" v-for="user in userList" :key="user.id" @click="selectUser(user)">
                                    <span>@{{ user.name }}</span>
                                </div>
                            </template>
                            <template v-else-if="triggerString === '#'">
                                <div class="trigger-item" v-for="tag in tagList" :key="tag.id" @click="selectTag(tag)">
                                    <span>#{{ tag.name }}</span>
                                </div>
                            </template>
                        </div>
            ...
            <div class="demo-block">
                <h3>插槽用法</h3>
                <el-x-sender ref="customSender" v-model="slotContent" placeholder="带有头部、前缀和底部的发送器..." clearable>
                    <template #header>
                    ...
                    <template #footer>
                        <div class="custom-footer">
                            <span>自定义底部区域</span>
                            <el-button-group>
                                <el-button size="mini" icon="el-icon-picture" type="text">图片</el-button>
                                <el-button size="mini" icon="el-icon-video-camera" type="text">视频</el-button>
                                <el-button size="mini" icon="el-icon-paperclip" type="text">附件</el-button>
                            </el-button-group>
                        </div>
                    </template>
                </el-x-sender>
                <div class="demo-controls">
                    <el-button size="small" type="primary" @click="toggleHeader">{{ showHeader ? '关闭头部' : '显示头部' }}</el-button>
                </div>
            </div>
        </el-card>
...
export default {
    data() {
        return {
            ...
            // AI对话模拟
            chatMessages: [{ type: 'bot', content: '你好！我是AI助手，有什么可以帮助你的？' }],
            chatInput: '',
            chatLoading: false,
        }
    },
