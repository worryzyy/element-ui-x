    ...
    <div>
        <el-card class="demo-card">
            ...
            <div class="demo-block">
                <h3>基础用法</h3>
                <el-x-conversations :items="basicItems" :active="activeConversation" @change="handleConversationChange" />
            ...
            <div class="demo-block">
                ...
                <div class="control-row">
                    <h4>列表高度：</h4>
                    <el-slider style="width: 200px;" v-model="listHeight" :min="200" :max="500" :step="50" />
                </div>
                <div class="grouped-container" :style="{height: `${listHeight}px`}">
                    <el-x-conversations :items="groupedItems" :active="activeGroupedConversation" :groupable="true" @change="handleGroupedChange" />
                </div>
            ...
            <div class="demo-block">
                <h3>自定义分组排序</h3>
                <el-x-conversations :items="customGroupedItems" :groupable="groupSortOptions" :active="activeCustomGrouped" @change="handleCustomGroupedChange" />
                <div class="demo-description">可以通过自定义排序函数控制分组的顺序，例如按字母排序或其他逻辑</div>
            </div>
            ...
            <div class="demo-block">
                ...
                <el-x-conversations :items="menuItems" :active="activeMenuItem" :show-built-in-menu="showBuiltInMenu" @change="handleMenuItemChange" @menu-command="handleMenuCommand" />
                <div class="demo-description">启用内置菜单后，鼠标悬停在项目上会显示操作菜单，点击菜单项触发对应操作</div>
                <div class="operation-log" v-if="operationLogs.length > 0">
                    <h4>操作记录：</h4>
                    <div v-for="(log, index) in operationLogs" :key="index" class="log-item">{{ log }}</div>
                </div>
            </div>
            ...
            <div class="demo-block">
                ...
                <div class="demo-controls">
                    ...
                    <div class="control-row">
                        <h4>自定义样式：</h4>
                        <el-form :inline="true" class="custom-css-form">
                            <el-form-item>
                                <el-autocomplete v-model="cssProperty" :fetch-suggestions="queryPropertySearch" placeholder="CSS属性" size="small" style="width: 140px;"></el-autocomplete>
                            </el-form-item>
                            <el-form-item>
                                <el-autocomplete v-model="cssValue" :fetch-suggestions="queryValueSearch" placeholder="CSS值" size="small" style="width: 140px;"></el-autocomplete>
                            </el-form-item>
                            <el-form-item>
                                <el-button size="small" type="primary" @click="addCustomCss">添加</el-button>
                            </el-form-item>
                        </el-form>
                    </div>
                    <div v-if="Object.keys(toTopBtnStyle).length > 0" class="applied-styles">
                        <div class="style-title">已应用样式：</div>
                        <el-tag v-for="(value, prop) in toTopBtnStyle" :key="prop" closable size="small" @close="removeStyle(prop)" class="style-tag">{{ prop }}: {{ value }}</el-tag>
                    </div>
                </div>
            </div>
                ...
                >
                    ...
                    <template #label="{item}">
                        <div class="custom-label">
                            <i :class="item.icon || 'el-icon-chat-dot-round'" class="custom-icon"></i>
                            <span class="custom-text">{{ item.label }}</span>
                            <span class="custom-time">{{ item.time }}</span>
                        </div>
                    </template>
                </el-x-conversations>
                <div class="demo-description">可以自定义项目样式、容器样式，以及通过插槽自定义分组标题和项目内容</div>
            </div>
            ...
            <div class="demo-block">
                    ...
                    <div class="customer-service-panel">
                        ...
                        <div class="panel-body">
                            <div class="sidebar">
                                ...
                                <div class="conversation-list" ref="serviceConversationList">
                                    <div v-if="loadingInitialConversations" class="conversation-loading">
                                        ...
                                        >
                                            <template #label="{item}">
                                                <div class="service-item">
                                                    <div class="user-avatar" :class="{'has-badge': item.unread > 0}">
                                                    ...
                                                    <div class="user-info">
                                                        ...
                                                        <div class="last-message" :class="{'is-unread': item.unread > 0}">{{ item.lastMessage }}</div>
                                                    </div>
                                                </div>
                                            </template>
                                        </el-x-conversations>
                            ...
                            <div class="chat-area">
                                <div v-if="activeServiceItemData" class="chat-header">
                                    <div class="chat-title">
                                        <span>{{ activeServiceItemData.label }}</span>
                                        <span class="user-status" :class="{'is-online': activeServiceItemData.online}">{{ activeServiceItemData.online ? '在线' : '离线' }}</span>
                                    </div>
                                    <div class="header-actions">
                                        <el-button size="mini" icon="el-icon-s-tools" type="text">设置</el-button>
                                    </div>
                                </div>
                                <div v-if="activeServiceItemData" class="chat-content">
                                    <div class="chat-message-placeholder">
                                        <div class="welcome-message">
                                            <el-alert title="会话已连接" type="success" description="您现在可以与客户沟通了，所有消息将被加密传输。" show-icon :closable="false"></el-alert>
                                ...
                                <div v-if="activeServiceItemData" class="chat-input">
                                    ...
                                    <el-input type="textarea" :rows="3" placeholder="请输入消息内容..." v-model="replyMessage"></el-input>
                                    <div class="send-btn">
                                        <el-button type="primary" icon="el-icon-s-promotion" :disabled="!replyMessage.trim()">发送</el-button>
                                    </div>
                                </div>
                                <div v-if="!activeServiceItemData" class="empty-chat">
                ...
                <div class="demo-description">这个案例模拟了客服系统中的会话列表实际应用场景，展示了组件如何与其他元素集成，以及如何处理未读消息、在线状态等实际功能，并使用了组件自带的懒加载功能自动加载更多会话内容</div>
            </div>
        </el-card>
    </div>
</template>
    ...
    data() {
        return {
            ...
            basicItems: [
                ...
                { id: '5', label: '最近对话5', prefixIcon: 'el-icon-chat-round', disabled: true },
            ],
            ...
            groupedItems: [
                ...
                { id: 'g3', label: '工作群3', group: '工作', prefixIcon: 'el-icon-office-building' },
                { id: 'g4', label: '学习小组1', group: '学习', prefixIcon: 'el-icon-reading' },
                { id: 'g5', label: '学习小组2', group: '学习', prefixIcon: 'el-icon-reading' },
                { id: 'g6', label: '同学聊天群', group: '学习', prefixIcon: 'el-icon-reading' },
                { id: 'g7', label: '家人群聊', group: '家庭', prefixIcon: 'el-icon-house' },
                { id: 'g8', label: '亲戚群', group: '家庭', prefixIcon: 'el-icon-house' },
                { id: 'g9', label: '朋友圈1', group: '朋友', prefixIcon: 'el-icon-user' },
                { id: 'g10', label: '朋友圈2', group: '朋友', prefixIcon: 'el-icon-user' },
                { id: 'g11', label: '朋友圈3', group: '朋友', prefixIcon: 'el-icon-user' },
                { id: 'g12', label: '未分类会话1', prefixIcon: 'el-icon-chat-line-round' },
                { id: 'g13', label: '未分类会话2', prefixIcon: 'el-icon-chat-line-round' },
            ],
            ...
            // 自定义分组排序
            customGroupedItems: [
                { id: 'c1', label: '项目A讨论', group: 'A级项目', prefixIcon: 'el-icon-s-flag' },
                { id: 'c2', label: '项目B规划', group: 'B级项目', prefixIcon: 'el-icon-s-flag' },
                { id: 'c3', label: '项目C评审', group: 'C级项目', prefixIcon: 'el-icon-s-flag' },
                { id: 'c4', label: '项目A需求', group: 'A级项目', prefixIcon: 'el-icon-s-flag' },
                { id: 'c5', label: '项目B进度', group: 'B级项目', prefixIcon: 'el-icon-s-flag' },
                { id: 'c6', label: '项目D立项', group: 'D级项目', prefixIcon: 'el-icon-s-flag' },
            ],
            groupSortOptions: {
                sort: (a, b) => {
                    // 按项目级别排序: A > B > C > D > 未分组
            ...
            // 自定义菜单
            customMenuItems: [
                { id: 'cm1', label: '重要客户A', prefixIcon: 'el-icon-user', suffixIcon: 'el-icon-star-on' },
            ...
            // 自定义样式
            styledItems: [
                { id: 's1', label: '设计讨论', group: '设计部', time: '10:30', icon: 'el-icon-picture-outline' },
                { id: 's2', label: 'UI评审', group: '设计部', time: '昨天', icon: 'el-icon-picture' },
                { id: 's3', label: '后端架构', group: '技术部', time: '周一', icon: 'el-icon-s-operation' },
                { id: 's4', label: 'API讨论', group: '技术部', time: '周二', icon: 'el-icon-s-platform' },
                { id: 's5', label: '产品规划', group: '产品部', time: '3天前', icon: 'el-icon-s-goods' },
                { id: 's6', label: '需求梳理', group: '产品部', time: '上周', icon: 'el-icon-document' },
            ],
            activeStyledItem: 's1',
            customItemsStyle: {
                borderRadius: '4px',
    ...
    methods: {
        ...
        getCssValueOptions() {
            ...
            setTimeout(() => {
                const valueMap = {
                    ...
                    color: [{ value: '#409EFF' }, { value: '#67C23A' }, { value: '#E6A23C' }, { value: '#F56C6C' }, { value: '#FFFFFF' }],
                    fontSize: [{ value: '12px' }, { value: '14px' }, { value: '16px' }, { value: '18px' }],
                    fontWeight: [{ value: 'normal' }, { value: 'bold' }, { value: '400' }, { value: '600' }],
                    position: [{ value: 'absolute' }, { value: 'fixed' }, { value: 'relative' }],
                    transform: [{ value: 'scale(1.1)' }, { value: 'rotate(45deg)' }, { value: 'translateY(-5px)' }],
        ...
        queryPropertySearch(queryString, cb) {
            ...
            const results = queryString
                ? properties.filter((prop) => prop.value.toLowerCase().indexOf(queryString.toLowerCase()) === 0)
                : properties
        ...
        queryValueSearch(queryString, cb) {
            ...
            if (this.cssProperty) {
                ...
                setTimeout(() => {
                    const results = queryString
                        ? this.cssValueOptions.filter((item) => item.value.toLowerCase().indexOf(queryString.toLowerCase()) === 0)
        ...
        generateMockConversations(page, pageSize) {
            ...
            // 时间描述
            const times = ['刚刚', '5分钟前', '10分钟前', '半小时前', '1小时前', '2小时前', '今天', '昨天', '前天', '上周', '两周前']
            ...
            for (let i = 0; i < pageSize; i++) {
                ...
                conversations.push({
                    ...
                    vipLevel:
                        unreadRandom > 0.8 ? '钻石会员' : unreadRandom > 0.6 ? '黄金会员' : unreadRandom > 0.3 ? '白银会员' : '普通会员',
                    registerTime: `${2020 + Math.floor(Math.random() * 4)}-${String(Math.floor(Math.random() * 12) + 1).padStart(
                        2,
                        '0'
                    )}-${String(Math.floor(Math.random() * 28) + 1).padStart(2, '0')}`,
                    lastPurchase:
                        unreadRandom > 0.2
                            ? `${products[productIndex]} ${['Pro', 'Plus', 'Max', 'Lite', ''][Math.floor(Math.random() * 5)]}`
                            : '未购买',
                })
            }
        ...
        loadMoreConversations(isInitial = false) {
            ...
            setTimeout(() => {
                // 生成新的会话数据
                const newConversations = this.generateMockConversations(this.conversationPage, this.conversationPageSize)
        ...
        handleConversationScroll(e) {
            ...
            // 当滚动到底部时，自动触发加载更多
            if (scrollHeight - scrollTop - clientHeight < 50 && this.hasMoreConversations && !this.loadingMoreConversations) {
